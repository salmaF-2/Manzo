...
//       const exists = conversations.some(c => c.participant._id === userId);
...
//  const filteredConversations = conversations.filter(conv => {
//   // Vérification complète de l'existence du participant
...
//                           <span className="text-xs text-gray-400 whitespace-nowrap ml-2">{conversation.lastMessageTime}</span>
//                         </div>
//                         <p className="text-sm text-gray-500 truncate">{conversation.lastMessage}</p>
//                       </div> */}
//                       <div className="relative">
...
const MessagesC = () => {
  ...
  useEffect(() => {
    ...
    //       ...message,
    //       sent: !isReceivedMessage // Inversez la logique si nécessaire
    //     }]);
    //   }
    // });
    socket.current.on("newMessage", (message) => {
    if (message.conversationId === activeConversation?._id) {
      ...
      // Vérifiez que le message n'existe pas déjà
