...
const Profil = () => {
  ...
  const formatDisponibilites = (disponibilites) => {
    if (!disponibilites || disponibilites.length === 0) {
        return [
            <div key="none" className="flex justify-between hover:text-blue-600 transition-colors duration-300">
                <span>Aucune disponibilité renseignée</span>
            </div>
        ];
    }

    return disponibilites.map((dispo, index) => {
        if (!dispo.jour || !dispo.heures || dispo.heures.length === 0) return null;
        
        const heures = dispo.heures[0];
        const [heureDebut, heureFin] = heures.split('-');

        if (!heureDebut || !heureFin) {
            return (
                <div key={index} className="flex justify-between hover:text-blue-600 transition-colors duration-300">
                    <span>{dispo.jour}</span>
                    <span className="text-red-500">Heures non définies</span>
                </div>
            );
        }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 transform transition-all hover:scale-[1.01] hover:shadow-xl">
            <div className="flex justify-between mb-4">
                ...
                <Link to="/modifierProfil" state={{ from: '/ProfilPrestataire' }}>
            ...
            <div className="text-gray-600 text-sm flex flex-col gap-2">
                {formatDisponibilites(userData.prestataireInfo?.disponibilites)}
            </div>
        </div>
        </div>
