...
const PopUp = ({ isOpen, onClose }) => {
  ...
  // Référence pour stocker les fichiers temporairement
  ...
  const handleFileChange = (file, fieldName) => {
    // Stocker dans la ref et dans le state
    if (fieldName === 'certifications') {
      const newCertifications = [...filesRef.current.certifications, file];
      filesRef.current.certifications = newCertifications;
  ...
  const handlePrev = () => {
    // Lorsqu'on revient en arrière, on récupère les fichiers depuis la ref
    setUploadedFiles({
      cin: filesRef.current.cin,
      rib: filesRef.current.rib,
      certifications: filesRef.current.certifications,
      carteAE: filesRef.current.carteAE,
      photoProfil: filesRef.current.photoProfil,
      videoPresentation: filesRef.current.videoPresentation
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Ajouter les fichiers depuis la ref (plus fiable)
      if (filesRef.current.cin) formDataToSend.append('cin', filesRef.current.cin);
      if (filesRef.current.rib) formDataToSend.append('rib', filesRef.current.rib);
      if (filesRef.current.carteAE) formDataToSend.append('carteAE', filesRef.current.carteAE);
      if (filesRef.current.photoProfil) formDataToSend.append('photoProfil', filesRef.current.photoProfil);
      if (filesRef.current.videoPresentation) formDataToSend.append('videoPresentation', filesRef.current.videoPresentation);

      // Ajouter les certifications
      if (filesRef.current.certifications && filesRef.current.certifications.length > 0) {
        filesRef.current.certifications.forEach(file => {
