...
const FileUpload = ({ 
  ...
  accept = 'image/*,.pdf',
  required = false,
  initialFile = null, // Nouvelle prop pour la valeur initiale
  initialPreview = null // Nouvelle prop pour la prévisualisation initiale
}) => {
  const [file, setFile] = useState(initialFile);
  const [preview, setPreview] = useState(initialPreview);
  const [error, setError] = useState(null);

  // Effet pour gérer les changements de initialFile
  useEffect(() => {
    if (initialFile) {
      setFile(initialFile);
      if (initialFile instanceof File && initialFile.type.startsWith('image/')) {
