...
import Knitra from "../assets/images/villes/Knitra.jpg"; // Corrected to Kenitra in mapping if necessary
import Oujda from "../assets/images/villes/Oujda.jpg";
import Tanger from "../assets/images/villes/Tangier.jpeg";
import Titouane from "../assets/images/villes/Titouan.jpg"; // Corrected to Tetouan in mapping if necessary
...
const VilleDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setServices(cityData.services || []);

        // Fetch reviews for the city
        const reviewsResponse = await fetch(
          `${API_BASE_URL}/reviews/city/${ville}`
        );
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-8 text-center text-lg">
        Chargement des détails de la ville...
      </div>
    );
  }
  ...
  if (!cityDetails) {
    return (
      <div className="container mx-auto px-4 py-8 text-center text-lg">
        Détails de la ville non trouvés pour {ville}.
      </div>
    );
  }
  ...
  const cityImageSrc = cityDetails.image
    ...
    : staticVilleImages[cityDetails.name] ||
      "https://via.placeholder.com/1200x400?text=Image+de+la+ville+non+trouvée";
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <section className="mb-12">
        ...
        {services.length > 0 ? (
                    ...
                    src={
                      service.image
                        ? service.image.startsWith("/uploads/")
                          ? `${BASE_SERVER_URL}${service.image}`
                          : `${BASE_SERVER_URL}/uploads/services/${service.image}`
                        : service.category &&
                          serviceImages[service.category.name]
                        ? serviceImages[service.category.name]
                        : "https://via.placeholder.com/400x300?text=Image+du+service"
                    }
                    alt={service.title}
                ...
                <div className="p-4">
                  ...
                  <p className="text-gray-700 text-sm mb-2">
                    ...
                    {service.prestataire?.nom ||
                      service.prestataire?.firstName ||
                      "N/A"}{" "}
                    {service.prestataire?.prenom ||
                      service.prestataire?.lastName ||
                      ""}
                  </p>
          ...
          <p className="text-gray-600 text-center mt-8">
            Aucun service disponible pour {cityDetails.name} pour le moment.
          </p>
        )}
      </section>
      ...
      <section className="mb-12">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800">
            Prestataires vérifiés à {cityDetails.name}
          </h2>
            ...
            .map((prestataire, index) => (
              ...
              >
                <div className="relative h-48">
                  <img
                    src={
                      prestataire.bannerImage
                        ? `${BASE_SERVER_URL}${prestataire.bannerImage}`
                        : prestataire.photo
                        ? `${BASE_SERVER_URL}${prestataire.photo}`
                        : serviceImages[prestataire.service] ||
                          staticVilleImages[cityDetails.name] ||
                          "https://via.placeholder.com/400x300?text=Image+de+prestataire"
                    }
                  ...
                  <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2">
                    <div className="w-12 h-12 rounded-full border-4 border-white bg-white shadow-md overflow-hidden">
                      <img
                        src={
                          prestataire.prestataireInfo?.documents?.photoProfil
                            ? `${BASE_SERVER_URL}${prestataire.prestataireInfo.documents.photoProfil}`
                            : "https://via.placeholder.com/100x100?text=Profil"
                        }
                        alt={
                          prestataire.name ||
                ...
                <div className="pt-8 pb-4 px-4 text-center">
                  ...
                  <div className="flex justify-center items-center mb-2">
                    ...
                    <span className="font-semibold">
                      {/* Displays the average rating from prestataireInfo */}
                      {prestataire.prestataireInfo?.noteMoyenne?.toFixed(1) || "N/A"}
                    </span>
                    <span className="text-gray-500 text-sm ml-1">
                      {/* Displays the number of reviews from prestataireInfo */}
                      ({prestataire.prestataireInfo?.nombreAvis || 0} avis)
                    </span>
                  </div>
                  <div className="text-sm text-gray-600">
                    <FaMapMarkerAlt className="inline mr-1" />
                    {/* prestataire.distance is not a standard field, will show N/A if not present */}
        ...
        {prestatairesInCity.length === 0 && (
          <p className="text-gray-600 text-center mt-8">
            Aucun prestataire de service trouvé pour {cityDetails.name}.
          </p>
        )}
      </section>
      ...
      {reviews.length > 0 && (
        <section className="mb-12">
          <h2 className="text-2xl font-bold text-gray-800 mb-6">
            Avis des clients à {cityDetails.name}
          </h2>
                ...
                <div className="flex items-center mt-4">
                  <div className="w-12 h-12 rounded-full overflow-hidden mr-4">
                    <img
                      src={
                        avis.profileImage
                          ? `${BASE_SERVER_URL}${avis.profileImage}`
                          : "https://via.placeholder.com/50x50?text=User"
                      }
                      alt={avis.nom || "Client Anonyme"}
                      className="w-full h-full object-cover"
                    />
                  </div>
